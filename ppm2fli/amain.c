...
static int print_help()
{
  ...
  -g<width>x<height>  Set width and height (10x10 - %dx%d),\n\
                        default is 640x480 (320x200 when using '-O')\n\
  -z<width>x<height>  Set width and height of input images for raw r8's\n\
                        this automatically sets r8 type to pure RAW--no header\n\
  ...
  -p<palettefile>     Sets palette file for r8 images\n",FLI_MAX_X,FLI_MAX_Y);
...
main (int argc, char *argv[])
{
  int j;
  FILE *fopen();
  char *listfile, *ppa, *ppb, *map_file, *output_pal_file, *geom,*inputgeom;
  char **pending_name;
  char abuff[10];
  ...
  for (i=1; i < argc; i++)
      ...
      else if ((*ppa == '-') || (*ppa == '+'))
	{
	  ...
	  switch (*ppa)
	    ...
	    case 'p':
	    case 'P':
	      GETNAME(palfname); palread=1; break;
	    case 'Q':
	      ppa++;
	      if (*ppa == 'c')
	    ...
	    case 'z':   GETNAME(inputgeom); break;
	    invalid_option:
	    default:
  ...
  if(palread==1){
    ...
    /* read palette file if r8 */
    if( (pal_file = fopen(palfname,"rb"))==NULL)
      {
	fprintf(stderr,"Couldn't open palette %s\n",palfname);
	print_hint();
	exit(1);
      }
    else{
      /*
	if( (pal_out = fopen("test.pal","wb"))==NULL)
	{
	...
	error(errortext);
      ...
      */
      for(j=0;j<3;j++){
	for(i=0;i<256;i++){
	  fread(&pal[j][i],sizeof(UBYTE),1,pal_file);
	  //fputc(pal[j][i],pal_out);
  ...
  if (inputgeom != NULL)
    {
      if ((sscanf(inputgeom,"%dx%d",&r8_width,&r8_height)) != 2)
	{
	  fprintf (stderr,"Invalid input geometry: '%s'\n",inputgeom);
	      print_hint();
	      exit (1);
	}
    }
  else{ r8_width=r8_height=0;}
