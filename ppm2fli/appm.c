...
static int get_r8_int(FILE *fp);
static int get_r8_bit(FILE *fp);
...
int get_r8_data(PMS *image, FILE *fp, int type)
{
  int yy;
  UBYTE *pp, *buff, *zz;
  UBYTE newval[FLI_MAX_COLORS];
  ...
  switch (type)
    ...
    case 10:
      raw_flag = 1;
      if (verbose_flag > 0) fprintf(stdout," r8 raw ");
      break;
    default:
      fprintf(stderr,"Invalid r8 file type\n");
      return(0);
    }
  ...
  if(r8_height==0){
    if ((image->width=get_r8_int(fp)) < 0) return(0);
    if ((image->height=get_r8_int(fp)) < 0) return(0);
    if (bw_flag)
      { maxval = 255; }
    else
      { if ((maxval=get_r8_int(fp)) < 0) return(0); }
  }
  else{
    image->width=r8_width;
    image->height=r8_height;
    maxval=255;
  }
  image->len = image->width * image->height;
  pix_len = 3 * image->len;
  ...
  buff_len = image->len; // force for r8
